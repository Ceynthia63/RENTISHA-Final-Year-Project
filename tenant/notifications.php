    ...
    <div class="card">
      <?php if (empty($notifications)): ?>
        <div class="empty-state">
          ...
          <p><?php if ($filter === 'unread'): ?>You're all caught up!<?php elseif ($filter === 'read'): ?>No read notifications yet.<?php else: ?>You don't have any notifications yet.<?php endif; ?></p>
        </div>
      <?php else: ?>
        <div class="notification-list">
          ...
          $notifIcons = [
              ...
              'unit_assigned'    => ['icon' => 'bi-house-check', 'color' => 'success', 'bg' => 'rgba(34,197,94,0.1)'],
              'application_approved' => ['icon' => 'bi-check-circle-fill', 'color' => 'success', 'bg' => 'rgba(34,197,94,0.1)'],
          ];
          foreach ($notifications as $n):
            $iconData = $notifIcons[$n['type']] ?? ['icon' => 'bi-bell', 'color' => 'primary', 'bg' => 'rgba(59,130,246,0.1)'];
            $isUnread = !$n['is_read'];
          ?>
          <div class="notification-item" style="display: flex; gap: 15px; padding: 16px; border-bottom: 1px solid var(--border); <?= $isUnread ? 'background: var(--primary-pale);' : '' ?>">
            ...
            <div style="flex: 1; min-width: 0;">
              <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 5px;">
                <h4 style="font-size: 0.95rem; margin: 0; <?= $isUnread ? 'font-weight: 700;' : 'font-weight: 500;' ?>">
                  ...
                  <?php if ($isUnread): ?><span class="badge badge-primary" style="margin-left: 8px; font-size: 0.65rem;">NEW</span><?php endif; ?>
                </h4>
                <span class="text-muted" style="font-size: 0.75rem; white-space: nowrap;"><?= formatDate($n['created_at']) ?></span>
              </div>
              <?php if (!empty($n['body'])): ?>
              <p style="margin: 0 0 8px 0; color: var(--text-muted); font-size: 0.85rem; line-height: 1.5;"><?= htmlspecialchars($n['body']) ?></p>
              <?php endif; ?>
              <div style="display: flex; gap: 10px; align-items: center;">
                <?php if (!empty($n['link'])): ?>
                <a href="<?= htmlspecialchars(notificationLink($n['link'])) ?>" class="btn btn-sm btn-outline"><i class="bi bi-arrow-right"></i> View Details</a>
                <?php endif; ?>
                <?php if ($isUnread): ?>
                <form method="POST" action="../includes/mark_notification_read.php" style="display: inline;">
                  ...
                  <button type="submit" class="btn btn-sm btn-text" style="color: var(--primary);"><i class="bi bi-check"></i> Mark as Read</button>
                </form>
                <?php endif; ?>
              </div>
            </div>
...
.notification-item:hover { background: var(--hover-bg) !important; }
.empty-state { text-align: center; padding: 60px 20px; }
.empty-state h3 { margin: 15px 0 5px; color: var(--text-dark); }
.empty-state p { color: var(--text-muted); }
.tabs { display: flex; gap: 5px; border-bottom: 2px solid var(--border); }
.tab { padding: 12px 20px; text-decoration: none; color: var(--text-muted); font-weight: 500; border-bottom: 3px solid transparent; transition: all 0.2s; display: flex; align-items: center; gap: 8px; }
.tab:hover { color: var(--text-dark); background: var(--hover-bg); }
